...
def build_provider(spec: dict[str, Any], items: ItemTable, base: Path) -> CandidateProvider:
    ...
    scorer = SCORERS[scorer_name] if scorer_name in SCORERS else _import_object(scorer_name)
    if kind == "python":
        from evaldelta.providers.callable import CallableProvider

        return CallableProvider(
            _import_object(spec["callable"]), scorer, retries=int(spec.get("retries", 2))
        )
    if kind == "command":
